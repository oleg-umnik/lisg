...
sub iptables_insert($$$$)
{
    ...
    my $success = $ipt->insert_rule("$insert_chain", "$the_match", "$target", "$target_opts");
    return $success;
}
...
sub iptables_append($$$$)
{
    ...
    my $success = $ipt->append_rule("$append_chain", "$the_match", "$target", "$target_opts");
    return $success;
}
...
sub iptables_delete($$$$)
{
    ...
    my $success = $ipt->delete_rule("$chain", "$the_match", "$target", "$target_opts");
    return $success;
}
...
#    }
    ...
    $self->iptables_delete($POS, "$ip", $target_chain, $target_opts);
    $self->iptables_insert($POS, "$ip", $target_chain, $target_opts);
...
#    if ( $self->iptables_chain_exist("$target_chain") ) {
	$success = $self->iptables_delete("$leaf_chainname", "$ip", "$target_chain", "$target_opts");
#    }
...
=head1 SVN revision
 ...
 $Date: 2006-08-09 15:35:21 +0200 (Wed, 09 Aug 2006) $
